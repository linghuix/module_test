/**
* @hardware: 正点原子MiniSTM32 V3.5. SPI1 连 W25Q64
*/
...
#define W25Q_TEST(...) 			MSG(__VA_ARGS__)
 ...
 * @para pBuffer:数据存储区
 * @para WriteAddr:开始写入的地址(24bit)
 * @para NumByteToWrite:要写入的字节数(最大65535)
//CHECK OK
*/
...
/*****************Test***************/
#ifdef W25QXX_TEST
u8 TEXT_Buffer[] ="你好hello233";//一个汉字代表两个字节
u8 RCV_Buffer[20];
u32 SIZE;
...
//	
	W25QXX_Write((u8*)TEXT_Buffer,0,sizeof(TEXT_Buffer));	
	//从倒数第100个地址处开始,写入SIZE长度的数据

	W25QXX_Read(RCV_Buffer,0,sizeof(TEXT_Buffer));  
	//从最小系统读取IMU数据 //从倒数第100个地址处开始,读出SIZE个字节
	
	RCV_Buffer[sizeof(TEXT_Buffer)] = 0;		//末尾加0，形成字符串
	
	for(int i=0;i<sizeof(TEXT_Buffer);i++){
		printf("%X ", RCV_Buffer[i]);
	}
